  ...
  describe "POST /api/v1/auth/password" do
    ...
    context "存在するメールアドレスの場合" do
      ...
      it "メール内のリンクは許可された redirect_url のホストを使う" do
        ...
        post "/api/v1/auth/password", params: { email: user.email, redirect_url: "#{allowed}/password/reset" }, as: :json
      ...
      it "不正な redirect_url は FRONTEND_BASE_URL にフォールバックする" do
        ...
        post "/api/v1/auth/password", params: { email: user.email, redirect_url: disallowed }, as: :json
