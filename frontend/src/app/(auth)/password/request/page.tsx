    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md text-center">
          ...
          <p className="mt-2 text-sm text-warm-brown-700">サービス設定が完了していないため、パスワードリセット機能は一時的に利用できません。問題が続く場合は管理者にお問い合わせください。</p>
        </div>
      </main>
    )
  }
  ...
  return (
    <main className="font-display bg-gradient-to-br from-warm-bg to-warm-brown-100 text-warm-brown-800 antialiased min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-8">
        <div>
          <h1 className="text-center text-3xl font-bold text-warm-brown-800">パスワードをリセット</h1>
          <p className="mt-2 text-center text-sm text-warm-brown-700">登録済みのメールアドレスを入力して、再設定用のリンクを受け取ってください。</p>
        </div>
        ...
        <div className="rounded-xl bg-warm-surface/80 backdrop-blur-sm p-8 shadow-soft ring-1 ring-warm-brown-200/50">
          {sent ? (
            <div className="space-y-4 text-center">
              ...
              <div>
                <a href="/login" className="text-warm-brown-700 font-medium hover:text-warm-orange">ログイン画面に戻る</a>
              </div>
            </div>
          ) : (
