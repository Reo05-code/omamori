				...
				<aside className="w-64 bg-surface-light dark:bg-surface-dark border-r border-border-light dark:border-border-dark flex-col justify-between hidden md:flex transition-colors duration-200 z-20 shadow-sm">
					<div>
						...
						<nav className="mt-6 px-3 space-y-1">
							<a className="group flex items-center px-4 py-3 text-sm font-medium bg-primary text-white rounded-lg shadow-md transition-all" href="#">
				...
				<div className="flex-1 flex flex-col min-w-0 overflow-hidden">
					<header className="h-16 bg-surface-light dark:bg-surface-dark border-b border-border-light dark:border-border-dark flex items-center justify-between px-4 sm:px-6 lg:px-8 z-10">
						...
						<div className="flex-1 flex justify-center lg:justify-start lg:ml-6">
							<div className="w-full max-w-lg lg:max-w-xs relative">
								...
								<input id="search" name="search" placeholder="作業員や現場を検索..." type="text" className="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md leading-5 bg-white dark:bg-gray-800 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-secondary focus:border-secondary sm:text-sm text-gray-900 dark:text-gray-100 transition duration-150 ease-in-out" />
							</div>
						</div>
					...
					<main className="flex-1 overflow-y-auto focus:outline-none p-4 sm:p-6 lg:p-8">
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
							<div className="bg-surface-light dark:bg-surface-dark overflow-hidden shadow-sm rounded-xl border border-border-light dark:border-border-dark flex flex-col p-6">
							...
							<div className="bg-primary overflow-hidden shadow-lg rounded-xl flex flex-col p-6 text-white ring-1 ring-black ring-opacity-5">
								<div className="flex items-center justify-between mb-4">
									<h3 className="text-sm font-medium text-gray-300">未対応アラート</h3>
									<span className="material-icons-outlined text-gray-300 text-2xl">notifications_active</span>
								</div>
								<div className="flex items-baseline">
									<span className="text-4xl font-bold">3</span>
								</div>
								<div className="mt-2 text-sm text-danger flex items-center font-semibold">
									<span className="material-icons-outlined text-base mr-1">warning</span>
									要対応
								</div>
							</div>
						...
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
							<div className="lg:col-span-2 bg-surface-light dark:bg-surface-dark shadow-sm rounded-xl border border-border-light dark:border-border-dark flex flex-col h-full min-h-[400px]">
										...
										<div className="flex-1 relative bg-gray-50 dark:bg-gray-900 overflow-hidden rounded-b-xl map-pattern">
											<div className="absolute top-1/2 left-1/3 transform -translate-x-1/2 -translate-y-1/2">
												<div className="bg-primary text-white w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-white dark:border-gray-800 z-10 relative">A</div>
												<div className="absolute top-full mt-2 w-48 bg-white dark:bg-gray-800 p-3 rounded shadow-lg border border-gray-200 dark:border-gray-700 text-center z-20">
											...
											<div className="absolute bottom-1/3 right-1/3 transform">
												<div className="bg-white dark:bg-gray-700 text-gray-800 dark:text-white w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-gray-300 dark:border-gray-600">B</div>
											</div>
										</div>
							</div>

							<div className="bg-surface-light dark:bg-surface-dark shadow-sm rounded-xl border border-border-light dark:border-border-dark flex flex-col">
...
function AlertItem({ name, time, text, variant = 'default' }: { name: string; time: string; text: string; variant?: 'default' | 'important' | 'muted' }) {
	...
	const classes = variant === 'important'
		? `${base} bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/50`
		: variant === 'muted'
	...
	const bar = variant === 'important' ? 'bg-primary' : variant === 'muted' ? 'bg-green-500' : 'bg-gray-400';
